...
// FeedbackToast: imperative component so feedback state changes don't re-render KorfbalApp
const FeedbackToast = React.forwardRef((_, ref) => {
...
export default function KorfbalApp() {
  ...
  const saveMatch = async (match) => {
    try {
      ...
      // Pre-validatie: controleer of gebruiker nog ingelogd is
      ...
      <div className="min-h-screen bg-gradient-to-br from-yellow-600 to-yellow-800 p-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-6 max-w-4xl mx-auto">
          ...
          {!teams ? (
            ...
            <>
              <div className="mb-6 p-4 bg-yellow-50 border-l-4 border-yellow-600 rounded flex items-center justify-between">
                <p className="font-semibold text-yellow-800">Totaal aantal teams: {teams.length}</p>
                <button onClick={() => {
                  showConfirm({
                    title: 'Dubbele teams verwijderen',
                    message: 'Alle teams met dezelfde naam worden samengevoegd. Per naam blijft het team met de meeste wedstrijden over. Dit kan niet ongedaan gemaakt worden.',
                    confirmLabel: 'Opschonen',
                    variant: 'danger',
                    onConfirm: async () => {
                      try {
                        const result = await cleanDuplicateTeamsMutation({});
                        showFeedback(`Opgeschoond: ${result.deleted} dubbele team(s) verwijderd`, 'success');
                      } catch (e) {
                        showFeedback('Fout bij opschonen: ' + (e.message || 'onbekend'), 'error');
                      }
                    }
                  });
                }} className="bg-yellow-600 text-white px-3 py-1 rounded text-sm hover:bg-yellow-700 ml-4 whitespace-nowrap">
